...
function loadUsers(usersList) {
    ...
    <th class="comp_name">CompetetionName</th>
    <th class="type">Type</th>
    <th class="part_name">ParticipantName</th>
    ...
    for (const user of ucom) {
        ...
        <td class="functions">
            <button class="edit" onclick="editUser(${user.comp_id});" type="button" id=${user.comp_id}><i class="fa-solid fa-pen-clip"></i></button><button class="deleteB" onclick="deleteUser(${user.comp_id});"type="button" id=${user.comp_id}><i class="fa-solid fa-trash-can"></i></button>
        </td>
    </tr>`;
 ...
 function fetchUsers() {
    ...
    xhttp.onreadystatechange= async function () {
        if(this.readyState==4){
            if(this.status==200){
            allUsers= await JSON.parse(this.response);
